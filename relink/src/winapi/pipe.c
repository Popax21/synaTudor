...
static void pipe_destroy(struct winpipe *pipe) {
    cant_fail(pthread_mutex_lock(&pipe->lock));
    if(--pipe->refcnt > 0) {
        cant_fail(pthread_mutex_unlock(&pipe->lock));
        return;
    }
    pipe->dying = true;

    //Remove from pipes list
    cant_fail(pthread_mutex_unlock(&pipes_lock));
    ...
    cant_fail(pthread_cond_broadcast(&pipe->listen_cond));
    while(pipe->num_listeners > 0) cant_fail(pthread_cond_wait(&pipe->listen_cond, &pipe->lock));

    //Free memory
    cant_fail(pthread_cond_destroy(&pipe->listen_cond));
    cant_fail(pthread_mutex_unlock(&pipe->lock));
    cant_fail(pthread_mutex_destroy(&pipe->lock));
...
__winfnc HANDLE CreateNamedPipeW(const char16_t *name, DWORD open_mode, DWORD pipe_mode, DWORD max_instances, DWORD out_buf_size, DWORD in_buf_size, DWORD default_timeout, void *security_attrs) {
    ...
    cant_fail(pthread_mutex_init(&pipe->lock, NULL));
    pipe->dying = false;

    pipe->num_listeners = 0;
    cant_fail(pthread_cond_init(&pipe->listen_cond, NULL));

    //Add to pipes list
    cant_fail(pthread_mutex_lock(&pipes_lock));
    pipe->prev = NULL;
...
__winfnc BOOL ConnectNamedPipe(HANDLE handle, OVERLAPPED *ovlp) {
    ...
    cant_fail(pthread_mutex_lock(&pipe->lock));
    while(!pipe->dying) cant_fail(pthread_cond_wait(&pipe->listen_cond, &pipe->lock));
    cant_fail(pthread_cond_broadcast(&pipe->listen_cond));
    cant_fail(pthread_mutex_unlock(&pipe->lock));
