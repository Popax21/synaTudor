...
static void req_destr(struct wdf_request *req) {
    ...
    //Free memory
    cant_fail(pthread_mutex_destroy(&req->lock));
    cant_fail(pthread_cond_destroy(&req->compl_cond));
...
struct wdf_request *wdf_create_request(struct wdf_object *parent, WDF_OBJECT_ATTRIBUTES *obj_attrs) {
    ...
    cant_fail(pthread_mutex_init(&req->lock, NULL));
    req->is_configured = req->is_started = req->is_done = false;
    req->status = STATUS_SUCCESS;
    cant_fail(pthread_cond_init(&req->compl_cond, NULL));
...
void wdf_reset_request(struct wdf_request *req) {
    cant_fail(pthread_mutex_lock(&req->lock));
    if(!req->is_configured || req->is_resetting) {
        while(req->is_configured && req->is_started && !req->is_done) cant_fail(pthread_cond_wait(&req->compl_cond, &req->lock));
        cant_fail(pthread_mutex_unlock(&req->lock));
        return;
    }
...
void wdf_complete_request(struct wdf_request *req, NTSTATUS status, WDF_REQUEST_COMPLETION_PARAMS *params) {
    ...
    cant_fail(pthread_cond_broadcast(&req->compl_cond));
...
NTSTATUS wdf_wait_request(struct wdf_request *req) {
    ...
    if(req->is_configured) {
        while(req->is_configured && !req->is_done) cant_fail(pthread_cond_wait(&req->compl_cond, &req->lock));
        status = req->status;
    }
